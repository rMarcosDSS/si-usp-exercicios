...
/*
Funcao auxiliar que recebe os parametros da funcao buscarPalavra e retorna um valor booleano, indicando se a palavra foi encontrada ou não, percorrendo recursivamente os nós da Trie conforme os caracteres da palavra.
*/
int buscarPalavraAux(PONT atual, char* palavra, int n, int posicao) {
    if (!atual) return 0;
    if (posicao == n) return atual->contador;

    int index = palavra[posicao] - VALOR_a;
    if (!atual->filhos || !atual->filhos[index]) return 0;

    return buscarPalavraAux(atual->filhos[index], palavra, n, posicao + 1);
}
...
/*
Funcao auxiliar que recebe os parametros da funcao inserirAux, ela percorre recursivamente a Trie criando novos nós conforme necessário e marca o final da palavra
*/
void inserirAux(PONT atual, char* palavra, int n, int posicao) {
    if (posicao == n) {
        atual->contador++;
        return;
    }
    ...
    if (!atual->filhos) {
        ...
        for (int i = 0; i < LETRAS; i++) atual->filhos[i] = NULL;
    }
    ...
    inserirAux(atual->filhos[index], palavra, n, posicao + 1);
}
...
/*
Funcao auxiliar que recebe os parametros da funcao excluirTodas, percorre a Trie para encontrar a palavra e, se encontrada, remove os nós correspondentes
*/
bool excluirAux(PONT atual, char* palavra, int n, int posicao) {
    ...
    if (atual->filhos && excluirAux(atual->filhos[index], palavra, n, posicao + 1)) {
        free(atual->filhos[index]);
        atual->filhos[index] = NULL;

        for (int i = 0; i < LETRAS; i++) {
            if (atual->filhos[i]) return false;
        }
...
void excluir(PONT raiz, char* palavra, int n){
  ...
  if (buscarPalavra(raiz, palavra, n) > 0) {
